...
program programadores;
    procedure leerDatos(var legajo: integer; var salario : real); // 1- los dos por referencia
    begin
        writeln('Ingrese el nro de legajo y el salario');
        read(legajo);
        read(salario);
    end;
    procedure actualizarMaximo(nuevoLegajo:integer; nuevoSalario:real; var maxLegajo:integer; var maxSalario: real); // 3- agregar maxSalario como parametro
    begin
        if (nuevoLegajo > maxLegajo) then begin
            maxLegajo:= nuevoLegajo;
            maxSalario := nuevoSalario
...
var
    legajo, maxLegajo, i : integer;
    salario, maxSalario, sumaSalarios : real; // 4- declarar variable sumaSalarios
begin
    sumaSalarios := 0;
    for i := 1 to 3 do begin
        leerDatos(legajo, salario);
        actualizarMaximo(legajo, salario, maxLegajo, maxSalario); // 5- pasar maxSalario como parametro de salida
        sumaSalarios := sumaSalarios + salario;
    end;
    writeln('En todo el mes se gastan ', sumaSalarios:0:2, ' pesos'); // en ambos casos agregar :0:2, para no mostrar tantos decimales1
